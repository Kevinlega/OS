	...
	def run(self): 										# Job to do
		...
		while True:
			...
			if localhistory != MAX_JOBS:			# Keep running until no more jobs can me put in queue
				...
				data, address = s.recvfrom(1024)				# Get the message from mobile in data and address of mobile in 
															# addr
				...
				addr.enqueue(address)						# Add connection to a queue
	...
	def run(self): 										# Job to do
		...
		while True:
				...
				if not localempty:						# If queue not empty means there is a job waiting						
					...
					address = addr.dequeue()			# Get connection to seend OK to after is consumed
					...
					mssg = bytes(("Consumer consumed your job %s"%data),"ascii")
					s.sendto(mssg,address)
...
MAX_JOBS = 30	   										# Nth job stop !!! Global not modified, only read; like C++ const
